<template>
  <div class="space-y-6">
      ...
      <div class="mb-4 flex flex-col gap-3">
        <div class="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
          ...
          <div class="flex flex-nowrap items-center gap-3 overflow-x-auto">
            ...
            <input v-model.trim="searchQuery" type="text" class="min-w-[260px] flex-1 rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none ring-cyan-500 focus:ring-2" />

            <div class="flex shrink-0 flex-nowrap gap-3 text-sm">
    ...
    <div v-if="showDeleteDialog" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/45 p-4" @click="closeDeleteDialog">
      <div class="w-full max-w-md overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl" @click.stop>
        <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4">
          <div>
            <h3 class="font-display text-lg font-bold text-slate-900">{{ text.deleteDialogTitle }}</h3>
          </div>
          <button class="inline-flex h-10 w-10 items-center justify-center rounded-full border border-slate-300 bg-white text-slate-700 shadow-sm hover:bg-slate-50" :disabled="deletingMarks" @click="closeDeleteDialog">
        ...
        <div class="px-5 py-4">
          <p class="text-sm leading-6 text-slate-600">{{ text.deleteDialogMessage }}</p>
        </div>

        <div class="flex items-center justify-end gap-3 border-t border-slate-200 px-5 py-4">
          <button class="rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50" :disabled="deletingMarks" @click="closeDeleteDialog">
            {{ text.cancel }}
          </button>
          <button class="rounded-xl bg-rose-600 px-4 py-2 text-sm font-semibold text-white hover:bg-rose-700 disabled:cursor-not-allowed disabled:opacity-60" :disabled="deletingMarks" @click="confirmDeleteMarks">
            {{ deletingMarks ? text.deleting : text.confirmDelete }}
          </button>
        </div>
      </div>
    </div>
