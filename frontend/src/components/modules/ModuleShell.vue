  ...
  <div class="space-y-5">
    ...
    <section v-if="supportsReports" class="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
      <div class="flex gap-2">
        <button class="rounded-lg px-4 py-2 text-sm font-semibold" :class="activeTab === 'view' ? 'bg-cyan-600 text-white' : 'bg-slate-100 text-slate-700'" @click="$emit('change-tab', 'view')">View</button>
        <button class="rounded-lg px-4 py-2 text-sm font-semibold" :class="activeTab === 'reports' ? 'bg-cyan-600 text-white' : 'bg-slate-100 text-slate-700'" @click="$emit('change-tab', 'reports')">Reports</button>
      </div>
    </section>
